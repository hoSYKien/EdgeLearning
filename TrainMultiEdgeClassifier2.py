...
MODEL_ROOT = r"D:\TongHop\RTC Technologi\PCB\model\model12"
...
def fit_head_logreg(X, y, num_classes, C, max_iter):
    ...
    if clf.coef_.shape[0] == 1:  # sklearn trả 1 hàng khi nhị phân
        # softmax([0, z]) == [1-sigmoid(z), sigmoid(z)] -> tương đương hoàn toàn
        W = np.vstack([np.zeros_like(clf.coef_[0]), clf.coef_[0]])
