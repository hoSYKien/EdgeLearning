...
5. Sau khi train xong, TỰ ĐỘNG sinh báo cáo Grad-CAM cho 1 số ảnh (ưu tiên
   ảnh đoán SAI trong out-of-fold) để biết model đang thực sự "nhìn" vào
   đâu để ra quyết định - không chỉ nhìn con số accuracy khô khan.
...
DATASET_DIRS = [
    r"D:\TongHop\RTC Technologi\PCB\crop5\train",
    r"D:\TongHop\RTC Technologi\PCB\crop5\val",
]

MODEL_DIR = r"D:\TongHop\RTC Technologi\PCB\model\model8"
...
K_FOLDS = 10            # số fold cross-validation - tăng nếu dataset lớn hơn
...
# --- Bao cao Grad-CAM sau khi train xong ---
GENERATE_GRADCAM_REPORT = True
GRADCAM_REPORT_DIR = os.path.join(MODEL_DIR, "gradcam_report")
# So anh Grad-CAM luu TOI DA cho MOI class - uu tien anh DOAN SAI (out-of-
# fold) truoc, neu con cho trong tren so nay moi lay them anh doan dung de
# lam mau doi chieu. Dat so lon neu muon xem het, nhung se lau hon.
...
def train_head(train_emb, train_labels, val_emb, val_labels, embedding_dim, num_classes,
    ...
    """Train 1 classifier head trên embedding đã cache."""
...
# ==========================================================================
# GRAD-CAM REPORT - sinh sau khi train xong, de biet model "nhin" vao dau
# ==========================================================================

class GradCAM:
    """Grad-CAM dung forward/backward hook - tai su dung y het gradcam_heatmap.py."""
...
def generate_gradcam_report(full_model, image_paths, labels_np, oof_pred, oof_probs,
                             class_names, report_dir, max_per_class, gradcam_transform):
    """Sinh anh Grad-CAM cho 1 so anh (uu tien anh doan SAI trong out-of-
    fold truoc, sau do lay them anh doan dung de doi chieu), luu vao
    report_dir + xuat 1 CSV tom tat. KHONG anh huong gi den model da train
    - day chi la buoc TRUC QUAN sau cung."""
    ...
    print(f"\nDang sinh Grad-CAM cho {len(selected)} anh (uu tien anh doan sai)...")
    rows = []
    for i in selected:
        path = image_paths[i]
        ...
        # QUAN TRONG: backbone dang bi dong bang (requires_grad=False tu
        # build_full_model). Neu input cung khong yeu cau gradient, PyTorch
        # se KHONG lan truyen duoc gradient qua cac lop conv dong bang -
        # khien hook cua GradCAM khong bao gio duoc goi (self.gradients se
        # mai la None). Bat requires_grad tren INPUT (khong dung cham den
        # trong so model) de "mo duong" cho gradient chay qua duoc.
        ...
        # Tinh Grad-CAM theo dung class model DA DOAN (de xem "model dua
        # vao dau de ra ket qua nay", ke ca khi ket qua do sai).
        cam, output = gradcam.compute(input_tensor, oof_pred[i])
        overlay = _overlay_heatmap(img_bgr, cam)

        label_text = f"that={true_cls} doan={pred_cls} ({status})"
        ...
        rows.append([fname, true_cls, pred_cls, status, out_path])

    csv_path = os.path.join(report_dir, "gradcam_report_summary.csv")
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["File", "Class_that", "Class_doan", "Dung_Sai", "Anh_gradcam"])
        writer.writerows(rows)

    print(f"Da luu {len(rows)} anh Grad-CAM + tom tat CSV tai: {report_dir}")
...
def main():
    ...
    # --------------------------------------------------------------------
    # BAO CAO GRAD-CAM - de biet model dang thuc su hoc duoc nhung gi
    # --------------------------------------------------------------------
    if GENERATE_GRADCAM_REPORT:
        generate_gradcam_report(
            full_model, image_paths, labels_np, oof_pred, oof_probs,
