...
Kiến trúc Master - Slave 2D:
  - Cam 1 (Color - Master): Trừ nền / mask vật thể, đọc Barcode và gán ID.
...
def main():
    ...
    print(f"\n[Cấu hình]")
    print(f"  - Master Camera (Mask & Detect): [{master_name.upper()}] ({cams[master_name].cam_info['model']})")
    ...
    print("Vui lòng giữ khung nhìn TRỐNG không có sản phẩm trong 3 giây...")
    detector_master.learn_background(cams[master_name], num_frames=30)
    ...
    try:
        while True:
            ...
            # 1. Phát hiện vật thể & Quét Barcode TRÊN MASTER CAMERA
            master_obj_candidates = detector_master.detect_objects(frame_master)
            master_detections = []
            for (cx, cy), (x1, y1, x2, y2) in master_obj_candidates:
                crop = frame_master[y1:y2, x1:x2]
                barcode = detector_master.decode_barcode(crop)
                master_detections.append(((cx, cy), barcode, (x1, y1, x2, y2)))
            ...
            for name, cam in cams.items():
                ...
                if f is not None:
                    ...
                    role_str = "MASTER" if name == master_name else "SLAVE"
                    header = f"[{name.upper()}: {role_str}] {cam.cam_info['model']} | FPS: {cam.fps:.1f}"
                    cv2.rectangle(canvas, (0, 0), (disp_w, 30), (20, 20, 20), -1)
                    cv2.putText(canvas, header, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1, cv2.LINE_AA)
            ...
            if key == ord('q') or key == 27:
                ...
                detector_master.learn_background(cams[master_name], num_frames=20)
