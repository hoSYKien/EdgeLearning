"""
==============================================================================
PIPELINE TỔNG HỢP V2: FEW-SHOT CLASSIFIER + OOD DETECTOR (2-IN-1)
(BỔ SUNG: HUẤN LUYỆN OOD BẰNG CẢ ẢNH GỐC LẪN ẢNH AUGMENTATION ĐA DẠNG GÓC)
==============================================================================
Tính năng mới trong V2:
1. Huấn luyện OOD Detector (Cosine Centroid & Mahalanobis) kết hợp CẢ Clean lẫn
   Augmentation (xoay, lật, nghiêng, crop, đổi màu).
2. Giúp các class có nhiều góc chụp (như Part7 chai đứng, chai nằm, chai nghiêng)
   không bị lệch tâm, tránh hiện tượng bắt nhầm thành UNKNOWN/Dị vật.
3. Tách tập Train/Val cấp độ ảnh gốc (Group-level split) chống rò rỉ dữ liệu (data leakage).
4. Tính Final Centroids trên toàn bộ không gian biến thể để mô hình ONNX & Joblib
   nhận diện siêu nhạy và bao quát mọi góc chụp thực tế.
==============================================================================
"""
...
RUN_TAG = "ood_aug"                   # Tag phụ gắn vào run_id (VD: "ood_aug")
...
AUGMENT_CONFIG = {
    ...
    "rotation_degrees": 30,           # Góc xoay ngẫu nhiên (có thể tăng lên 45 nếu vật thể xoay nhiều)
    "color_jitter": True,
...
# 4. Cấu hình Huấn luyện OOD Detector (MỚI: BẬT AUGMENTATION CHO OOD)
OOD_USE_AUGMENTATION = True           # True = Dùng cả ảnh Augment + Clean để tính Centroid/Mahalanobis & Calibrate ngưỡng
ID_VAL_RATIO = 0.30                   # Phần trăm ID giữ lại để calibrate ngưỡng
OOD_TEST_RATIO = 0.50                 # Phần trăm OOD giữ lại để test độc lập
TARGET_TPR = 0.95                     # Mục tiêu giữ 95% mẫu ID chuẩn (có thể chỉnh 0.90 nếu muốn nới lỏng)
...
# ============================================================================
# MAIN PIPELINE 2-IN-1 (V2: OOD VỚI AUGMENTATION)
# ============================================================================

def main():
    print("=" * 90)
    print(" BẮT ĐẦU QUY TRÌNH HUẤN LUYỆN FEW-SHOT CLASSIFIER + OOD DETECTOR V2")
    print(" (ĐÃ TÍCH HỢP AUGMENTATION CHO CẢ BỘ LỌC OOD DETECTOR)")
    print("=" * 90)
    ...
    # 6. Huấn luyện OOD Detector (KẾT HỢP CẢ CLEAN VÀ AUGMENTATION)
    print("\n--- [Bước 4/4] Huấn luyện OOD Detector (Cosine & Mahalanobis) ---")
    have_ood = X_ood_all.shape[0] >= 2

    # Tách train/val trên cấp độ chỉ số ảnh gốc (Group Split) để đảm bảo không rò rỉ dữ liệu
    indices = np.arange(len(id_paths))
    try:
        train_img_idx, val_img_idx = train_test_split(
            indices, test_size=ID_VAL_RATIO, random_state=RANDOM_SEED, stratify=labels_np
        )
    except ValueError:
        train_img_idx, val_img_idx = train_test_split(
            indices, test_size=ID_VAL_RATIO, random_state=RANDOM_SEED
        )

    if OOD_USE_AUGMENTATION:
        # Tập Fit OOD: Kết hợp cả Clean và toàn bộ bản Augment của train_img_idx
        fit_embs = []
        fit_lbls = []
        for idx in train_img_idx:
            # Vector Clean gốc
            fit_embs.append(clean_emb_list[idx])
            fit_lbls.append(labels_np[idx])
            # Toàn bộ vector Augment (xoay, nghiêng, lật, crop,...)
            fit_embs.append(augmented_emb_list[idx])
            fit_lbls.extend([labels_np[idx]] * AUGMENT_COPIES)

        X_fit = torch.cat(fit_embs, dim=0).numpy()
        y_fit = np.array(fit_lbls)

        # Tập Val OOD: Kết hợp Clean và Augment của val_img_idx để calibrate ngưỡng thực tế
        val_embs = []
        for idx in val_img_idx:
            val_embs.append(clean_emb_list[idx])
            val_embs.append(augmented_emb_list[idx])
        X_val = torch.cat(val_embs, dim=0).numpy()

        print(f"  [OOD Augmentation = BẬT]")
        print(f"    -> Tập Fit OOD: {len(X_fit)} vectors ({len(train_img_idx)} ảnh gốc x {1 + AUGMENT_COPIES} mẫu)")
        print(f"    -> Tập Val OOD: {len(X_val)} vectors ({len(val_img_idx)} ảnh gốc x {1 + AUGMENT_COPIES} mẫu)")
    else:
        # Chế độ cũ: Chỉ dùng ảnh Clean gốc
        X_fit = torch.cat([clean_emb_list[i] for i in train_img_idx], dim=0).numpy()
        y_fit = np.array([labels_np[i] for i in train_img_idx])
        X_val = torch.cat([clean_emb_list[i] for i in val_img_idx], dim=0).numpy()
        print(f"  [OOD Augmentation = TẮT] (Fit: {len(X_fit)}, Val: {len(X_val)})")

    if have_ood:
        X_ood_val, X_ood_test = train_test_split(X_ood_all, test_size=OOD_TEST_RATIO, random_state=RANDOM_SEED)
        print(f"    -> Tập OOD-val={len(X_ood_val)} | OOD-test={len(X_ood_test)}")
    else:
        X_ood_val = X_ood_test = np.zeros((0, embedding_dim), np.float32)
        print(f"    -> (Không có đủ ảnh OOD để test AUROC)")

    # Fit mô hình OOD trên tập Fit
    ...
    # TÍNH TOÁN FINAL CENTROIDS & MAHALANOBIS TRÊN TOÀN BỘ DỮ LIỆU ID (ALL CLEAN + ALL AUGMENT)
    # Giúp mô hình suy luận bao phủ trọn vẹn nhất mọi góc độ trong thực tế
    print("  -> Đang tổng hợp Final Centroids trên toàn bộ dataset (Clean + Augment)...")
    if OOD_USE_AUGMENTATION:
        all_embs_list = []
        all_lbls_list = []
        for i in range(len(id_paths)):
            all_embs_list.append(clean_emb_list[i])
            all_lbls_list.append(labels_np[i])
            all_embs_list.append(augmented_emb_list[i])
            all_lbls_list.extend([labels_np[i]] * AUGMENT_COPIES)
        X_all_final = torch.cat(all_embs_list, dim=0).numpy()
        y_all_final = np.array(all_lbls_list)
        final_centroids = fit_centroids(X_all_final, y_all_final, num_classes)
        final_maha_means, final_maha_prec = fit_mahalanobis(X_all_final, y_all_final, num_classes)
    else:
        X_all_final = torch.cat(clean_emb_list, dim=0).numpy()
        y_all_final = labels_np
        final_centroids = fit_centroids(X_all_final, y_all_final, num_classes)
        final_maha_means, final_maha_prec = fit_mahalanobis(X_all_final, y_all_final, num_classes)

    # 7. Đóng gói & Lưu trữ Artifacts (Model .pt, Payload .joblib, JSON, ONNX)
    ...
    if EXPORT_ONNX:
        try:
            ...
            unified_model_cpu = UnifiedClassifierOODModel(full_model_cpu, final_centroids, best_threshold).eval()
    ...
    ood_payload = {
        ...
        "mahalanobis": {"means": final_maha_means, "precision": final_maha_prec},
        "target_tpr": TARGET_TPR,
        "metrics": ood_results,
        "ood_use_augmentation": OOD_USE_AUGMENTATION,
    }
    ...
    np.savez(
        ood_npz_path,
        centroids=final_centroids.astype(np.float32),
        maha_means=final_maha_means.astype(np.float32),
        maha_precision=final_maha_prec.astype(np.float32),
    ...
    print("\n" + "=" * 90)
    print(" TỔNG KẾT HOÀN TẤT V2")
    print(f" Run ID          : {run_id}")
    print(f" Thư mục Run     : {run_dir}")
    print(f" PyTorch Model   : {model_pt_path}")
    print(f" OOD Detector    : {ood_joblib_path}")
    print(f" OOD Augment     : {'BẬT (Clean + Augment)' if OOD_USE_AUGMENTATION else 'TẮT (Chỉ Clean)'}")
