...
from embedding import YOLOEmbedding
from matcher import Matcher

# cls_model = YOLO(r'D:/VS_Code/Training/Classifical/best.pt')
# CONF_THRESH = 0.98
extractor = YOLOEmbedding(r'D:/VS_Code/Training/Classifical/best.pt')
matcher = Matcher("database.pkl",threshold=14.0)
...
def classify_object(img):
    ...
    label, distance = matcher.predict(feature)
...
belt_mask = np.zeros((roi_h, roi_w), np.uint8)
cv2.fillPoly(belt_mask,[points_roi],255)
...
COLOR_PROFILE_WIDTH = 7      # bề rộng dải lấy mẫu
COLOR_JUMP_THRESH = 25       # ngưỡng khác màu

# ================================
# Hàm kiểm tra màu 
def has_color_boundary(roi_bgr, p1, p2,
                       band_width=COLOR_PROFILE_WIDTH,
                       color_thresh=COLOR_JUMP_THRESH):

    """
    p1,p2: (y,x)

    Trả True nếu dọc đường nối có bước nhảy màu rõ rệt.
    """

    h, w = roi_bgr.shape[:2]

    ys = np.linspace(p1[0], p2[0], 60)
    xs = np.linspace(p1[1], p2[1], 60)
    ...
    for y, x in zip(ys, xs):
        ...
        y1 = max(0, y-band_width)
        y2 = min(h, y+band_width+1)

        x1 = max(0, x-band_width)
        x2 = min(w, x+band_width+1)

        patch = roi_bgr[y1:y2, x1:x2]

        if patch.size == 0:
            continue

        colors.append(np.mean(patch.reshape(-1,3), axis=0))
    ...
    diff = np.linalg.norm(colors[1:] - colors[:-1], axis=1)

    return diff.max() > color_thresh
#===================================================================
...
def unlock_camera_settings(camera):
    pass  # Hàm cũ để mở khóa camera RealSense, không còn sử dụng

def lock_camera_settings(camera):
    pass  # Hàm cũ để khóa camera RealSense, không còn sử dụng
...
def watershed_split(blob_mask, roi_bgr):
    ...
    for c in coords:
        ...
        for k in kept:
            ...
            #     break
            neck = has_neck(dist, tuple(c), tuple(k))

            color = has_color_boundary(
                roi_bgr,
                tuple(c),
                tuple(k)
            )

            if not (neck and color):
                merged = True
                break
        if not merged:
            kept.append(tuple(c))
    ...
    try:
        while True:
            ...
            for b in bboxes:
                ...
                # label, conf = classify_object(crop)
                label, distance = classify_object(crop)
                # màu hiển thị
                color = (0,255,0) if label != "Non-object" else (0,0,255)
