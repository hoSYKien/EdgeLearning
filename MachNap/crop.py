...
5. ĐỒNG THỜI cắt luôn ảnh ROI (đã xoay thẳng, kích thước chuẩn chung) cho
   TỪNG ảnh mẫu, lưu ra thư mục riêng - để bạn xem/dùng ngay không cần
   chờ chạy script 2.
...
DUONG_DAN_THU_MUC_ANH_MAU = r"D:\TongHop\RTC Technologi\PCB\1"      # thư mục chứa NHIỀU ảnh mẫu
DUONG_DAN_LUU_TEMPLATE = r"D:\TongHop\RTC Technologi\PCB\1\template.pkl"    # nơi lưu bộ template
DUONG_DAN_THU_MUC_CROP_MAU = r"D:\TongHop\RTC Technologi\PCB\crop4"  # nơi lưu ảnh ROI đã cắt của CHÍNH các ảnh mẫu
# =====================================================================
...
def tao_bo_template(thu_muc_anh_mau: str, duong_dan_luu: str, thu_muc_crop_mau: str):
    ...
    # *** Cắt luôn ROI cho từng ảnh mẫu ***
    # Kích thước chuẩn chung = trung vị kích thước ROI đo được của mọi ảnh
    # mẫu, để tất cả ảnh cắt ra (kể cả sau này ở script 2) luôn cùng size.
    canonical_w = int(round(np.median([t["roi"][2] for t in templates])))
    canonical_h = int(round(np.median([t["roi"][3] for t in templates])))
    print(f"\nKích thước ROI chuẩn chung (trung vị): {canonical_w}x{canonical_h}")

    os.makedirs(thu_muc_crop_mau, exist_ok=True)
    print(f"Đang cắt ROI cho từng ảnh mẫu, lưu vào: {thu_muc_crop_mau}/\n")

    for tpl in templates:
        duong_dan = os.path.join(thu_muc_anh_mau, tpl["ten_file"])
        img = cv2.imread(duong_dan)
        x, y, w, h = tpl["roi"]

        warped = cv2.warpAffine(img, tpl["m_align"], tpl["ref_size"])
        ...
        ten_goc, _ = os.path.splitext(tpl["ten_file"])
        duong_dan_crop = os.path.join(thu_muc_crop_mau, f"{ten_goc}_roi.png")
        cv2.imwrite(duong_dan_crop, crop)
        print(f"  [{tpl['ten_file']}] -> {duong_dan_crop}")

    print(f"\nXong. Đã cắt {len(templates)} ảnh ROI vào thư mục: {thu_muc_crop_mau}/")


if __name__ == "__main__":
    tao_bo_template(DUONG_DAN_THU_MUC_ANH_MAU, DUONG_DAN_LUU_TEMPLATE, DUONG_DAN_THU_MUC_CROP_MAU)