...
BẢN SỬA so với bản gốc, giải quyết 4 vấn đề đã trao đổi:
  ...
  [Mục 7] TRAIN NHIỀU MODEL TRONG 1 LẦN CHẠY: DATASET_DIRS giờ là 1 MẢNG
          2 CHIỀU (list of list) - mỗi HÀNG là 1 model riêng biệt (ví dụ
          mỗi ROI/vùng lỗi trên PCB), các phần tử trong hàng đó là các
          thư mục train/val chứa các thư mục class con. Toàn bộ pipeline
          (K-Fold, train model cuối, Grad-CAM...) sẽ chạy lần lượt cho
          từng hàng, mỗi hàng ra 1 file model .pt + 1 báo cáo riêng, lưu
          vào thư mục con riêng trong MODEL_DIR (đặt tên tự động theo tên
          thư mục dataset của hàng đó).
...
# [Mục 7] MẢNG 2 CHIỀU: mỗi HÀNG = 1 model riêng. Trong 1 hàng, liệt kê các
# thư mục (train/val...) mà bên trong chứa các thư mục con là CLASS của
# model đó. Muốn train bao nhiêu model thì thêm bấy nhiêu hàng.
DATASET_DIRS_LIST = [
    [
        r"D:\TongHop\RTC Technologi\PCB\crop7\part1\train",
        r"D:\TongHop\RTC Technologi\PCB\crop7\part1\val",
    ],
    [
        r"D:\TongHop\RTC Technologi\PCB\crop7\part2\train",
        r"D:\TongHop\RTC Technologi\PCB\crop7\part2\val",
    ],
    [
        r"D:\TongHop\RTC Technologi\PCB\crop7\part3\train",
        r"D:\TongHop\RTC Technologi\PCB\crop7\part3\val",
    ],
    [
        r"D:\TongHop\RTC Technologi\PCB\crop7\part4\train",
        r"D:\TongHop\RTC Technologi\PCB\crop7\part4\val",
    ],
    [
        r"D:\TongHop\RTC Technologi\PCB\crop7\part5\train",
        r"D:\TongHop\RTC Technologi\PCB\crop7\part5\val",
    ]
]

MODEL_BASE_DIR = r"D:\TongHop\RTC Technologi\PCB\model\model13"
...
AUGMENT_CONFIG = {
    ...
    "rotation_degrees": 10,        # giảm từ 20 -> 10, đặt 0 để tắt hẳn
...
# ==========================================================================
# [Mục 7] Đặt tên thư mục model tự động theo dataset của từng hàng
# ==========================================================================

def derive_run_name(dataset_dirs, row_index):
    """Tự động đặt tên cho 1 hàng trong DATASET_DIRS_LIST, dùng làm tên
    thư mục con lưu model/báo cáo. Ưu tiên lấy tên thư mục cha của thư mục
    đầu tiên trong hàng (vd: '...\\roi_1_split\\train' -> 'roi_1_split').
    Nếu không tách được tên hợp lệ, dùng 'model_{row_index+1}'."""
    if dataset_dirs:
        first_dir = dataset_dirs[0].rstrip("\\/")
        parent_name = os.path.basename(os.path.dirname(first_dir))
        if parent_name:
            return f"{row_index + 1:02d}_{parent_name}"
    return f"model_{row_index + 1:02d}"


# ==========================================================================
# TRAIN 1 MODEL (toàn bộ pipeline cho 1 HÀNG trong DATASET_DIRS_LIST)
# ==========================================================================

def train_one_model(dataset_dirs, model_dir, run_label=""):
    ...
    print("\n" + "#" * 100)
    print(f"### TRAIN MODEL: {run_label}  ->  {model_dir}")
    print("#" * 100)
    ...
    print_detailed_report(image_paths, labels_np, oof_pred, oof_probs, oof_fold, class_names, model_dir)
    ...
    if CALIBRATE_TEMPERATURE:
        ...
        print_detailed_report(image_paths, labels_np, oof_pred, oof_probs_calibrated, oof_fold,
                               class_names, model_dir, temperature=temperature)
...
# ==========================================================================
# MAIN - [Mục 7] Lặp qua TỪNG HÀNG trong DATASET_DIRS_LIST, mỗi hàng train
# ra 1 model riêng. Nếu 1 hàng bị lỗi (vd thiếu thư mục), in lỗi và tự
# động chạy tiếp các hàng còn lại thay vì dừng toàn bộ.
# ==========================================================================

def main():
    print(f"Sẽ train tổng cộng {len(DATASET_DIRS_LIST)} model, mỗi model ứng với 1 hàng trong DATASET_DIRS_LIST.")

    summary = []
    for row_index, dataset_dirs in enumerate(DATASET_DIRS_LIST):
        run_name = derive_run_name(dataset_dirs, row_index)
        model_dir = os.path.join(MODEL_BASE_DIR, run_name)

        try:
            result = train_one_model(dataset_dirs, model_dir, run_label=run_name)
            summary.append((run_name, result))
        except Exception as e:
            print(f"\n!!! LỖI khi train model '{run_name}': {e}")
            print("!!! Bỏ qua model này, tiếp tục với model tiếp theo...\n")
            summary.append((run_name, None))

    print("\n\n" + "=" * 100)
    print("TỔNG KẾT TOÀN BỘ CÁC MODEL ĐÃ TRAIN")
    print("=" * 100)
    for run_name, result in summary:
        if result is None:
            print(f"- {run_name}: THẤT BẠI (xem lỗi ở log phía trên)")
        else:
            print(f"- {run_name}: K-Fold acc = {result['kfold_val_acc_mean']:.2%} "
                  f"± {result['kfold_val_acc_std']:.2%} | Final val acc = {result['final_model_val_acc']:.2%} "
                  f"| Model: {result['model_out']}")
