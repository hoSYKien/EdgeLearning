...
INPUT_DIR = r"D:\TongHop\RTC Technologi\HZT.Bottom\val\OK"
...
def _select(mask, img_area):
    ...
    # Anchor (điểm neo) PHẢI lấy từ nhóm sạch viền - tránh chọn nhầm nền làm anchor.
    anchor = max(clean, key=cv2.contourArea)
    box = cv2.boundingRect(anchor)

    # Gộp thêm các mảnh gần anchor - lấy từ TOÀN BỘ contour hợp lệ (valid), KHÔNG đòi hỏi mảnh đó
    # cũng phải sạch viền, vì mảnh nằm sát 1 anchor đã xác thực thì chắc chắn cùng 1 vật, kể cả
    # khi bản thân mảnh đó (vd: chân pin) chạm mép ảnh.
    boxes = [cv2.boundingRect(c) for c in valid]
    areas = [cv2.contourArea(c) for c in valid]
    used = {i for i, c in enumerate(valid) if c is anchor}
    changed = True
    while changed:
        changed = False
...
def detect_bbox(img):
    ...
    for inv in (False, True):
        ...
        _, m = cv2.threshold(gray, 0, 255, t + cv2.THRESH_OTSU)
